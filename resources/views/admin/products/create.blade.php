    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.products.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('clents') ? 'is-invalid' : '' }}" name="clents[]" id="clents" multiple>
                    @foreach($clents as $id => $clent)
                        <option value="{{ $id }}" {{ in_array($id, old('clents', [])) ? 'selected' : '' }}>{{ $clent }}</option>
                    @endforeach
                </select>
