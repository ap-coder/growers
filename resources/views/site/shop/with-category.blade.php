        ...
        <div class="row m-b30">
            <div class="col-12">
                <div class="category-filter-area">
                    <ul class="nav nav-pills category-pills flex-wrap justify-content-center">
                        <li class="nav-item">
                            <a class="nav-link {{ !request('category') ? 'active' : '' }}" href="{{ route('site.shop.index') }}">
                                All Products
                            </a>
                        </li>
                        @foreach($categories as $category)
                            <li class="nav-item">
                                <a class="nav-link {{ request('category') == $category->id ? 'active' : '' }}" href="{{ route('site.shop.index', ['category' => $category->id]) }}">
                                    {{ $category->name }}
                                </a>
                            </li>
                        @endforeach
